(function () {
  const QUESTIONS = [
    { tag: 'Q1 · IMPULSE CHECK', prompt: 'How did this item end up in your cart?', options: [
    ...
    { tag: 'Q2 · DUPLICATE CHECK', prompt: 'Do you already own something that does this job?', options: [
      { label: "No, there's a genuine gap", value: 0, flag: 'it fills a genuine gap' },
      { label: 'Yes, something similar already works', value: 2, flag: 'you already own something that does this' }
    ]},
    { tag: 'Q3 · SALES PRESSURE', prompt: 'Is anything on the page pushing you to decide fast?', options: [
    ...
    { tag: 'Q4 · AFFORDABILITY', prompt: 'Could you pay for this right now without checking your balance?', options: [
    ...
    { tag: 'Q5 · 30-DAY TEST', prompt: 'Will you still want this in 30 days?', options: [
    ...
    { tag: 'Q6 · OUTSIDE VIEW', prompt: 'If a friend said they were buying this, what would you tell them?', options: [
      { label: 'Good for them — smart buy', value: 0, flag: 'a friend would call it a smart buy' },
      { label: "I'd gently ask if they needed it", value: 1, flag: 'a friend would ask a few questions first' },
  ...
  function answer(question, option) {
    ...
    qIndex++;
    if (qIndex < QUESTIONS.length) { renderQuestion(); } else { finish(); }
  }
  ...
  function getVerdict(score) {
    if (score <= 2) { return { key: 'buy', label: 'Buy it', msg: 'This reads like a plan, not an impulse. Go ahead.' }; }
    if (score <= 6) { return { key: 'wait', label: 'Wait 24h', msg: "Nothing's wrong with it — but sleep on it. Revisit tomorrow with a clear head." }; }
  ...
  function reset() {
    ...
    el.priceInput.disabled = false;
    el.itemField.classList.remove('locked');
    el.priceField.classList.remove('locked');
    el.tape.replaceChildren();
    el.tagline.textContent = 'no purchases were made in the making of this receipt';
    el.stage.replaceChildren(el.startBtn);
    stampMeta();
    el.itemInput.focus();
  }
})();